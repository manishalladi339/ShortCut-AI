...
async def apply_plan(
    *,
    ...
    if plan["project_state_version"] != expected_version:
        raise _conflict("AI plan was generated for an older ProjectState version", state["version"])
    if plan.get("status") != "proposed":
        raise HTTPException(
            status_code=409,
            detail={"error": {"code": "planner.plan_not_proposed", "message": "Only proposed plans can be applied"}},
        )
    ...
    for operation in plan.get("operations", []):
        ...
        sequence = next(
            (item for item in candidate["sequences"] if item["id"] == payload["sequence_id"]),
            None,
        )
        if not sequence:
        ...
        if not track or track["kind"] != "video":
            raise HTTPException(status_code=422, detail="Plan target video track no longer exists")
        if track.get("locked"):
            raise HTTPException(
                status_code=409,
                detail={"error": {"code": "edit.track_locked", "message": "Target video track is locked"}},
            )

        key = (sequence["id"], track["id"])
        if key not in touched_tracks:
                ...
                raise HTTPException(
                    ...
                    detail={
                        "error": {
                            ...
                            "message": "Target video track already has clips; explicitly allow replacement",
                        }
                    },
                )
