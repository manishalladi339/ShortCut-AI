...
async def build_plan(*, project: dict, user_id: str, state: dict, body: CreateAIEditPlanRequest) -> dict:
    ...
    for candidate in sorted(chosen, key=lambda item: (item["asset_id"], item["start"])):
        if _candidate_key(candidate) not in seen:
            candidate["narrative_role"] = (narrative.get("roles") or {}).get(_candidate_key(candidate), "body")
            ordered.append(candidate)
    ...
    for candidate in ordered:
        ...
        broll = recommend_broll_for_highlight(highlight=candidate, visual_candidates=visual_candidates, highlight_vector=candidate.pop("semantic_vector"), limit=3)
        if broll:
