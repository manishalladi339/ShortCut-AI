...
async def _audit(user_id: str | None, action: str, metadata: dict | None = None) -> None:
    db = get_db()
...
async def signup(body: SignupBody) -> AuthResponse:
    ...
    if await db.users.find_one({"email": email}, {"_id": 0, "id": 1}):
        raise HTTPException(
            ...
            detail={"error": {"code": "auth.email_taken", "message": "Email already registered"}},
        )
    user_doc = {
        "id": str(uuid.uuid4()),
        "email": email,
...
async def login(body: LoginBody) -> AuthResponse:
    ...
    if not user or not user.get("password_hash"):
        raise HTTPException(
            ...
            detail={"error": {"code": "auth.invalid_credentials", "message": "Invalid email or password"}},
        )
...
async def refresh_tokens(body: RefreshBody) -> TokenPair:
    ...
    session = await db.sessions.find_one(
        {"refresh_token_hash": token_hash, "revoked": False}, {"_id": 0}
    )
    if not session:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail={"error": {"code": "auth.session_revoked", "message": "Session no longer valid"}},
        )
...
async def logout(body: RefreshBody, _user: dict = Depends(get_current_user)) -> dict:
    db = get_db()
    token_hash = hashlib.sha256(body.refresh_token.encode()).hexdigest()
    await db.sessions.update_one({"refresh_token_hash": token_hash}, {"$set": {"revoked": True}})
    return {"ok": True}
...
async def reset_password(body: ResetPasswordBody) -> dict:
    ...
    if not rec:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail={"error": {"code": "auth.invalid_reset_token", "message": "Invalid or used token"}},
        )
    exp = rec["expires_at"]
    ...
    if exp < utc_now():
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail={"error": {"code": "auth.expired_reset_token", "message": "Token expired"}},
        )
